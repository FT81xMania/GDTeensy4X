...
#define LOW_FREQ_BOUND  58800000UL//47040000UL
...
//FT81xmania Team
  ...
  if ((EEPROM.read(0) != 0x1)) {
  self_calibrate();
      for (int i = 0; i < 24; i++) 
        {EEPROM.write(1 + i, GDTR.rd(REG_TOUCH_TRANSFORM_B + i));
        //EEPROM.write(24+1 + i, GDTR.rd(REG_TOUCH_TRANSFORM_B + i));
        }
        EEPROM.write(0, 0x1);  
    } else {
         for (int i = 0; i < 24; i++)
         {GDTR.wr(REG_TOUCH_TRANSFORM_B + i, EEPROM.read(1 + i));
         //GDTR.wr(REG_TOUCH_TRANSFORM_B + i, EEPROM.read(24+1 + i));
         }
   }
